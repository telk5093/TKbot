 ...
 * Say
 */
var say = exports.say = async (data, message) => {
    switch (data.platform) {
        ...
        case 'cime':
            await data.callback[data.method]({
                'message': message,
            });
            break;
    }
...
var startWithCmd = exports.startWithCmd = (text, match, case_sensative) => {
    ...
    for (let i in match) {
        let str = String(match[i]).trim();
        if ((!case_sensative && text.substr(0, str.length).toLowerCase() === str.toLowerCase()) || (case_sensative && text.substr(0, str.length) === str)) {
            if (text.substring(str.length, str.length + 1) === ' ') {
                param = text.substr(str.length);
            } else {
                param = null;
            }
            return {
...
var isIn = exports.isIn = (text, match, case_sensative) => {
    ...
    for (let i in match) {
        ...
        if (case_sensative) {
            if (text.indexOf(_match) >= 0) {
                return _match;
            }
        } else {
            if ((text.toLowerCase()).indexOf(_match.toLowerCase()) >= 0) {
                return _match;
            }
        }
    }
...
var htmlspecialchars = exports.htmlspecialchars = (str) => {
    return String(str)
        .replace(/&/g, "&amp;")
        .replace(/</g, "&lt;")
        .replace(/>/g, "&gt;")
...
// More accurate random() (Node.js)
var random = exports.random = () => {
    // Uint32 범위의 난수 생성 후 0~1로 정규화
...
//     let recent_chatters = [];
//     let _time = time();
//     for (let user_id in chatters) {
//         if (_time - chatters[user_id].time < time_limit) {
//             recent_chatters.push(user_id);
//         }
//     }
